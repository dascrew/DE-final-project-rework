...
class TestSalesRecordIdFix:
    ...
    def test_sales_record_id_is_column(self):
        ...
        assert result["sales_record_id"].iloc[0] == 1, "First record should have sales_record_id = 1"
    ...
    def test_sales_record_id_increments_correctly(self):
        ...
        assert list(result["sales_record_id"]) == [1, 2], "sales_record_id should be [1, 2]"
    ...
    def test_output_column_order(self):
        ...
        assert result.columns[0] == "sales_record_id", "sales_record_id should be first column"
...
class TestLoadParquetDataErrorHandling:
    ...
    def test_parquet_read_error_raises(self, mock_wr, mock_retrieve):
        """Verify parquet read errors are raised, not silently ignored."""
        from lambda_load.src.load_parquet_data import read_parquet_data_to_dataframe
...
class TestWarehouseLoadFunctions:
    ...
    def test_default_schema_is_public(self):
        ...
        from lambda_load.src.warehouse_load_functions_pg8000 import load_data_into_warehouse
        import inspect

        sig = inspect.signature(load_data_into_warehouse)
        default_schema = sig.parameters["schema"].default

        assert default_schema == "public", f"Default schema should be 'public', got '{default_schema}'"
